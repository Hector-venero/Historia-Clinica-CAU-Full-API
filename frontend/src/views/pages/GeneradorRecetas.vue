...
async function buscarPacientesBD(event) {
  if (!event.query.trim()) { sugerenciasPacientes.value = []; return }
  try {
...
async function buscarMedicamento(event) {
  if (!event.query.trim()) { sugerenciasMed.value = []; return }
  try {
    ...
    <div class="max-w-3xl mx-auto space-y-8">
      ...
      <div class="bg-white dark:bg-[#1e1e1e] rounded-xl shadow p-6 space-y-4">
        <h2 class="text-base font-semibold text-gray-700 dark:text-gray-200 flex items-center gap-2">
          <i class="pi pi-user text-primary-500"></i> Datos del Paciente
        </h2>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div class="flex flex-col gap-1 sm:col-span-2">
            ...
            <Select
              v-model="financiadorSeleccionado"
              :options="financiadores"
              optionLabel="nombreComercial"
              optionValue="idfinanciador"
      ...
      <div class="bg-white dark:bg-[#1e1e1e] rounded-xl shadow p-6 space-y-4">
        <h2 class="text-base font-semibold text-gray-700 dark:text-gray-200 flex items-center gap-2">
          <i class="pi pi-heart text-primary-500"></i> Medicamento
        </h2>
        <AutoComplete
      ...
      <div class="bg-white dark:bg-[#1e1e1e] rounded-xl shadow p-6 space-y-3">
        <h2 class="text-base font-semibold text-gray-700 dark:text-gray-200 flex items-center gap-2">
          <i class="pi pi-list text-primary-500"></i> Diagnóstico (CIE-10)
        </h2>
        <AutoComplete
      ...
      <!-- Mensajes -->
      <div v-if="errorEmision" class="text-red-500 text-sm font-medium flex items-center gap-2">
        <i class="pi pi-exclamation-triangle"></i> {{ errorEmision }}
      </div>
      <div v-if="mensajeExito" class="text-green-600 text-sm font-medium flex items-center gap-2">
