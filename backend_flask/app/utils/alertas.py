...
def obtener_agenda_manana(usuario_id, fecha_manana):
    ...
    try:
        ...
        """
        cursor.execute(query_ind, (usuario_id, fecha_manana))
        turnos_individuales = cursor.fetchall()
        
        # 2. Buscar turnos grupales
        query_grup = """
        ...
        """
        cursor.execute(query_grup, (usuario_id, fecha_manana))
        turnos_grupales = cursor.fetchall()
        
    except Exception as e:
        current_app.logger.error(f"Error al obtener agenda para usuario {usuario_id}: {str(e)}")
        raise e
    finally:
        cursor.close()
        conn.close()
...
def procesar_y_enviar_alertas(dry_run=False):
    ...
    profesionales = []
    try:
        # Se buscan usuarios activos que tengan disponibilidad cargada y activa para el día de la semana de mañana
        query_profesionales = """
        ...
        """
        cursor.execute(query_profesionales, (dia_nombre,))
